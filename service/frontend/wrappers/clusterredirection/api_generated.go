...
func (handler *clusterRedirectionHandler) ListAllWorkflowExecutions(ctx context.Context, lp1 *types.ListAllWorkflowExecutionsRequest) (lp2 *types.ListAllWorkflowExecutionsResponse, err error) {
	var apiName = "ListAllWorkflowExecutions"
	var cluster string

	scope, startTime := handler.beforeCall(metrics.DCRedirectionListAllWorkflowExecutionsScope)
	defer func() {
	...
	err = handler.redirectionPolicy.WithDomainNameRedirect(ctx, lp1.GetDomain(), apiName, func(targetDC string) error {
		...
		switch {
		case targetDC == handler.currentClusterName:
			lp2, err = handler.frontendHandler.ListAllWorkflowExecutions(ctx, lp1)
		default:
			remoteClient := handler.GetRemoteFrontendClient(targetDC)
			lp2, err = remoteClient.ListAllWorkflowExecutions(ctx, lp1, handler.callOptions...)
		}
		return err
	})
