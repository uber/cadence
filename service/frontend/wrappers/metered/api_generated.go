...
func (h *apiHandler) ListAllWorkflowExecutions(ctx context.Context, lp1 *types.ListAllWorkflowExecutionsRequest) (lp2 *types.ListAllWorkflowExecutionsResponse, err error) {
	defer func() { log.CapturePanic(recover(), h.logger, &err) }()
	tags := []tag.Tag{tag.WorkflowHandlerName("ListAllWorkflowExecutions")}
	tags = append(tags, toListAllWorkflowExecutionsRequestTags(lp1)...)
	scope := h.metricsClient.Scope(metrics.FrontendListAllWorkflowExecutionsScope).Tagged(metrics.DomainTag(lp1.GetDomain())).Tagged(metrics.GetContextTags(ctx)...)
	scope.IncCounter(metrics.CadenceRequests)
	sw := scope.StartTimer(metrics.CadenceLatency)
	defer sw.Stop()
	logger := h.logger.WithTags(tags...)

	lp2, err = h.handler.ListAllWorkflowExecutions(ctx, lp1)
	if err != nil {
		return nil, h.handleErr(err, scope, logger)
	}
	return lp2, err
