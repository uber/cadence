...
To find out which **binary checksum** of bad deployment to reset, you should be aware of at least one workflow running into a bad state. Use describe command with **--reset_points_only** option to show all the reset points:
```
...
Then use this command to tell Cadence auto-reset all workflows impacted by the bad deployment. The bad binary checksum needs to store in domain info.
```
