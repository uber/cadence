...
The Cadence CLI is a command-line tool to perform various tasks on a Cadence server. It can perform 
...
`make bins`. This should produce an executable called `cadence`. With a local build, the same command to 
...
[here]({{ '/docs/02_key_features#durable-timers' | relative_url }}).
```
