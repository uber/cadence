...
If a bad deployment let a workflow run into a wrong state, you might want to reset the workflow to the point that the bad deployment started to run. But that usually not easy to find out all the workflows impacted, and every reset point for each workflow. In this case, auto-reset will automatically reset all the workflow by the bad deployment ID given. 

First of all, let's familar with some concepts. Each deployment will have a identifier, we call it "Binary Checksum" as it is usually generated by the md5sum of the binary file. For a workflow, each binary checksum will be associated with an **auto-reset point**, which contains a **runID**, an **eventID**, and the **created_time** that binary made the first decision for the workflow.

To find which **binary checksum** of bad deployment to reset, you should be aware of at least one workflow running into bad state. Using describe command with **--reset_points_only** option to show all the reset points:
```
...
Then use this command to tell Cadence reset all workflows impacted by the bad deployment. The bad binary checksum needs to store in domain info.
```
