...
buildkite-agent artifact download ".build/coverage/integ_cassandra_ev2_cover.out" . --step ":golang: integration test events v2 with cassandra" --build "$BUILDKITE_BUILD_ID"
...
buildkite-agent artifact download ".build/coverage/integ_sql_ev2_cover.out" . --step ":golang: integration test events v2 with mysql" --build "$BUILDKITE_BUILD_ID"
