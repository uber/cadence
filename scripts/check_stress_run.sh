...
#ROWSLINE=$(cqlsh $host --cqlversion=3.4.0 -e "use cadence; select * from executions where type=1 and run_id=dcb940ac-0c63-ffa2-ffea-a6c305881d71 LIMIT 10 allow filtering;" | grep "rows")
#if [ "$ROWSLINE" != "(0 rows)" ]; then
#    echo "Warning: Left over workflow executions ..."
#    cqlsh $host --cqlversion=3.4.0 -e "use cadence; select count(*) from executions where type=1 and run_id=dcb940ac-0c63-ffa2-ffea-a6c305881d71 LIMIT 10 allow filtering;"
#    cqlsh $host --cqlversion=3.4.0 -e "use cadence; select * from executions where type=1 and run_id=dcb940ac-0c63-ffa2-ffea-a6c305881d71 LIMIT 10 allow filtering;"
#else
